...
if ( ! function_exists( 'dbmngr_add_admin_menu' ) ) {
	function dbmngr_add_admin_menu() {
		...
		add_submenu_page( 'bws_plugins', __( 'DB manager Settings', 'dbmngr' ), 'DB manager', 'manage_options', 'db-manager.php', 'dbmngr_settings_page' );
	}
}
...
if ( ! function_exists( 'register_dbmngr_settings' ) ) {
	function register_dbmngr_settings() {
		...
		if ( ! get_option( 'dbmngr_options' ) )
			add_option( 'dbmngr_options', $dbmngr_option_defaults );
	...
	function dbmngr_curl_download( $dbmngr_url, $dbmngr_file ) {
		if ( function_exists( 'curl_init' ) ) {
			...
			if ( $dbmngr_fp = @fopen( $dbmngr_file, 'w' ) ) { 
				...
				$dbmngr_error = __( 'Could not create file!', 'dbmngr' ) . ' '. $dbmngr_file . '<br>';
			}
		}
	}
}
...
if ( ! function_exists( 'dbmngr_off_access_pma' ) ) {
	function dbmngr_off_access_pma( $dbmngr_path ) {
		...
		if ( is_writable( $dbmngr_create_name ) ) {
			...
			$dbmngr_error = __( 'Disable function does not work! File is not writable!', 'dbmngr' ) . ' ' . $dbmngr_create_name . '<br>';
		}
	}
}
...
if ( ! function_exists( 'dbmngr_file_access' ) ) {
	function dbmngr_file_access( $dbmngr_path, $dbmngr_path_pwd, $dbmngr_auth_name ) {
		...
		if ( is_writable( $dbmngr_create_name ) ) {
			...
			$dbmngr_error = __( 'Enable function does not work! File is not writable.', 'dbmngr' ) . ' ' . $dbmngr_create_name . '<br>';
		}
	}
}
...
if ( ! function_exists( 'dbmngr_file_password' ) ) {
	function dbmngr_file_password( $dbmngr_path, $key ) {
		...
		if ( $dbmngr_file_handle = @fopen( $dbmngr_create_name, 'w' ) ) {
			...
			$dbmngr_error = __( 'Impossible to open the file!', 'dbmngr' ) . ' '. $dbmngr_create_name . '<br>';
		}
	}
}
			...
			if ( isset( $dbmngr_option_array['user_name'] ) && isset( $dbmngr_option_array['user_password_dumper'] ) && isset( $dbmngr_option_array['catalog_dumper'] ) ) {
				...
				if ( '1' == $dbmngr_option_array['access_dumper'] ) {
					if ( 'true' == dbmngr_detect_ie() ) {
						...
						$dbmngr_link_dumper = ' <a href="' . plugin_dir_url( __FILE__ ) . '/dumper/' . $dbmngr_option_array['catalog_dumper'] . '/sxd/index.php" target=_blank>' . __( 'Access', 'dbmngr' ) . '</a> ';
					} else {
						$dbmngr_plugin_url = trim( preg_replace( '#^http(s)?://#', '', plugin_dir_url( __FILE__ ) ), '/' );
						$dbmngr_link_dumper = ' <a href="http://' . $dbmngr_option_array['user_name'] . ':' . $dbmngr_option_array['user_password_dumper'] . '@' . $dbmngr_plugin_url . '/dumper/' . $dbmngr_option_array['catalog_dumper'] . '/sxd/index.php" target=_blank>' . __( 'Access', 'dbmngr' ) . '</a> ';
					}
				}
			}
			...
			if ( isset( $dbmngr_option_array['user_name'] ) && isset( $dbmngr_option_array['user_password_phpmyadmin'] ) && isset( $dbmngr_option_array['catalog_phpmyadmin'] ) ) {
				...
				if ( '1' == $dbmngr_option_array['access_phpmyadmin'] ) {
					if ( 'true' == dbmngr_detect_ie() ) {
						...
						$dbmngr_link_phpmyadmin = ' <a href="' . plugin_dir_url( __FILE__ ) . '/phpmyadmin/' . $dbmngr_option_array['catalog_phpmyadmin'] . '/' . $dbmngr_str . '/index.php" target=_blank>' . __( 'Access', 'dbmngr' ) . '</a> ';
					} else {
						$dbmngr_plugin_url = trim( preg_replace( '#^http(s)?://#', '', plugin_dir_url( __FILE__ ) ), '/' );
						$dbmngr_link_phpmyadmin = ' <a href="http://' . $dbmngr_option_array['user_name'] . ':' . $dbmngr_option_array['user_password_phpmyadmin'] . '@' . $dbmngr_plugin_url . '/phpmyadmin/' . $dbmngr_option_array['catalog_phpmyadmin'] . '/' . $dbmngr_str . '/index.php" target=_blank>' . __( 'Access', 'dbmngr' ) . '</a> ';
					}
				}
			}
